...
class ShuffleNetV2_OneShot_newdataset(nn.Module):

    def __init__(self, input_size=224, n_class=100):
        super(ShuffleNetV2_OneShot_newdataset, self).__init__()
        ...
        self.globalpool = nn.AvgPool2d(7)
        self.dropout = nn.Dropout(0.1)
        self.classifier_newdataset = nn.Sequential(
            nn.Linear(self.stage_out_channels[-1], n_class, bias=False))
        self._initialize_weights()
    ...
    def forward(self, x, architecture):
        ...
        x = self.dropout(x)
        x = x.contiguous().view(-1, self.stage_out_channels[-1])
        x = self.classifier_newdataset(x)
        return x
