...
from network import ShuffleNetV2_OneShot, ShuffleNetV2_OneShot_newdataset
...
class EvolutionSearcher(object):
    ...
    def __init__(self, args, new_dataset=False):
        self.args = args
        ...
        if new_dataset:
            self.model = ShuffleNetV2_OneShot_newdataset()
        else:
            self.model = ShuffleNetV2_OneShot()
        self.model = torch.nn.DataParallel(self.model).cuda()
        supernet_state_dict = torch.load('')['state_dict']
        self.model.load_state_dict(supernet_state_dict, strict=False)
        ...
        self.checkpoint_name = os.path.join(self.log_dir, 'test_new_12.pth.tar')
        self.visit_epoch = []
        self.memory = []
        self.vis_dict = {}
    ...
    def is_legal(self, cand):
        ...
        info['err'] = get_cand_err(self.model, cand, self.args, self.optimizer)
...
def main():
    ...
    parser.add_argument('--max-train-iters', type=int, default=20)
    parser.add_argument('--max-test-iters', type=int, default=25)
    ...
    searcher = EvolutionSearcher(args, new_dataset=False)
